...
def load_cached_features(tanks_path, resnet_model):
    # Check if cache exists for each feature type
    cached_features = {}
    if os.path.exists(os.path.join(CACHE_DIR, 'features.pkl')):
...
# Load or cache dataset features
dataset_feats = load_cached_features(tanks_folder, resnet_model)

# Predict the tank in the input image
